...
const DndFilterSelect = (props: DndFilterSelectProps) => {
  ...
  >({});
  const [templateColumns, setTemplateColumns] = useState<ColumnMeta[]>([]);
  useEffect(() => {
    SupersetClient.get({
      endpoint: `/api/v1/dataset/${datasource.id}/template_columns`,
    })
      .then(({ json }) => {
        const templateColumnsMeta = ((json?.columns as string[]) || []).map(
          column => ({
            column_name: column,
            filterable: true,
            warning_markdown:
              'This column is generated by the SQL template and will not be saved with the chart.',
          }),
        );
        setTemplateColumns(templateColumnsMeta);
      })
      .catch(error => {
        logging.error(error);
      });
  ...
  const optionsForSelect = useCallback(
    (columns: ColumnMeta[], formData: QueryFormData | null | undefined) => {
